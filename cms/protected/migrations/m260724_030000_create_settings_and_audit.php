...
class m260724_030000_create_settings_and_audit extends CDbMigration
{
    ...
    public function up()
    {
        ...
        $this->createIndex('uniq_site_settings_setting_key', 'site_settings', 'setting_key', true);
        $this->createIndex('idx_site_settings_group_name', 'site_settings', 'group_name');

        // Bảng append-only. Không sửa, không xoá. Cân nhắc partition theo tháng
        // khi dữ liệu lớn.
        $this->createTable('audit_logs', array(
        ...
        $this->createIndex('idx_audit_logs_entity', 'audit_logs', array('entity_type', 'entity_id'));
        $this->createIndex('idx_audit_logs_user_id_created_at', 'audit_logs', array('user_id', 'created_at'));
        $this->createIndex('idx_audit_logs_created_at', 'audit_logs', 'created_at');
        $this->addForeignKey('fk_audit_logs_users', 'audit_logs', 'user_id',
