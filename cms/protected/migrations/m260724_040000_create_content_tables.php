...
class m260724_040000_create_content_tables extends CDbMigration
{
    ...
    {
        // ---------------------------------------------------- Section 1: Hero
        $this->createTable('pvn_hero_slides', array(
        ...
        $this->createIndex('idx_hero_slides_sort_order', 'pvn_hero_slides', 'sort_order');
        $this->createIndex('idx_hero_slides_is_active', 'pvn_hero_slides', 'is_active');
        $this->addForeignKey('fk_hero_slides_bg_media', 'pvn_hero_slides', 'background_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');
        $this->addForeignKey('fk_hero_slides_logo_media', 'pvn_hero_slides', 'logo_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');
        ...
        $this->createTable('pvn_business_sectors', array(
            'id'                => 'INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY',
        ...
        $this->createIndex('uniq_business_sectors_slug', 'pvn_business_sectors', 'slug', true);
        $this->createIndex('idx_business_sectors_sort_order', 'pvn_business_sectors', 'sort_order');
        $this->addForeignKey('fk_business_sectors_image_media', 'pvn_business_sectors', 'image_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');
        $this->addForeignKey('fk_business_sectors_icon_media', 'pvn_business_sectors', 'icon_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');

        // ------------------------------------------- Section 6: Giá trị cốt lõi
        $this->createTable('pvn_core_values', array(
            'id'             => 'INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY',
        ...
        $this->createIndex('idx_core_values_sort_order', 'pvn_core_values', 'sort_order');
        $this->addForeignKey('fk_core_values_icon_media', 'pvn_core_values', 'icon_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');

        // ------------------------------------- Section 7: Hành trình phát triển
        $this->createTable('pvn_timeline_milestones', array(
            'id'             => 'INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY',
        ...
        $this->createIndex('idx_timeline_milestones_year_value', 'pvn_timeline_milestones', 'year_value');
        $this->createIndex('idx_timeline_milestones_sort_order', 'pvn_timeline_milestones', 'sort_order');
        $this->addForeignKey('fk_timeline_milestones_media', 'pvn_timeline_milestones', 'image_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');

        // ------------------------------- Section 8: Đối tác & Cổ đông chiến lược
        $this->createTable('pvn_partners', array(
            'id'                => 'INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY',
        ...
        $this->createIndex('idx_partners_type_sort', 'pvn_partners', array('partner_type', 'sort_order'));
        $this->addForeignKey('fk_partners_logo_media', 'pvn_partners', 'logo_media_id',
            'pvn_media_files', 'id', 'SET NULL', 'CASCADE');
    }

    public function down()
    {
        $this->dropTable('pvn_partners');
        $this->dropTable('pvn_timeline_milestones');
        $this->dropTable('pvn_core_values');
        $this->dropTable('pvn_business_sectors');
        $this->dropTable('pvn_hero_slides');
    }
}
